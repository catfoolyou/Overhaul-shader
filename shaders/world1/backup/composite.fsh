...
vec3 GetLightmapColor(in vec2 Lightmap){
    ...
    const vec3 TorchColor = vec3(1.00, 0.92, 0.72);
    vec4 Albedo = texture2D(texture, TexCoords);
    // Add the lighting togther to get the total contribution of the lightmap the final color.
    return (Lightmap.x * TorchColor * 1.5) + (Lightmap.y * vec3(0.73, 0.00, 0.93));
}
...
void main(){
    ...
    #define Diffuse Albedo * (LightmapColor + 0.2) // lmcoord.y
    /* DRAWBUFFERS:0 */
